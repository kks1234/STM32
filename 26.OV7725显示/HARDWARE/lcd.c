...
void LCD_Set_Window(u16 sx,u16 sy,u16 width,u16 height)
{
	...
	LCD->LCD_RAM=(sx+width-1)>>8;
	LCD->LCD_RAM=(sx+width-1)&0x00FF;
	LCD->LCD_REG=lcd_dev.setycmd;
	LCD->LCD_RAM=sy>>8;
	LCD->LCD_RAM=sy&0x00FF;
	LCD->LCD_RAM=(sy+height-1)>>8;
	LCD->LCD_RAM=(sy+height-1)&0x00FF;
...
void LCD_Scan_Dir(u8 dir)
{
	...
	if(lcd_dev.id==0x5310)
	{
		...
		LCD_WR_DATA((lcd_dev.width-1)>>8);LCD_WR_DATA((lcd_dev.width-1)&0XFF);
		LCD_WR_REG(lcd_dev.setycmd); 
		LCD_WR_DATA(0);LCD_WR_DATA(0);
		LCD_WR_DATA((lcd_dev.height-1)>>8);LCD_WR_DATA((lcd_dev.height-1)&0XFF);  
